...
main:
#Inicializo los vectores y sus dimensiones en registos salvados
    la $s0, v1
    la $s1, v2
    li $s2, 0
    li $s3, 0
    li $t1, 0 # Multiplicar el size
# Empieza el vector desde el 10 así que empezare en ese numero a sumar
    li.s $f4, 10.0
    li.s $f8, 1.0
    li $t8, 4
# En este for relleno el vector 1 de valores y lo hago 40 veces
    for1:
        mul $t0, $t8, $t1
        addu $t2, $s0, $t0
        s.s $f4, 0($t2)
        ...
        blt $t1, 40, for1
    forfin:
# Empieza el vector desde el 40 así que empezare en ese numero a restar 1
    li.s $f4, 40.0
    li $t1, 0
    li.s $f8, -1.0
# En este for relleno el vector 2 de valores y lo hago 40 veces
    for2:
        mul $t0, $t8, $t1
        addu $t9, $s1, $t0
        s.s $f4, 0($t9)
...
cambiar_dim:
# Eliges el vector que quieras cambiar de dimension
    li $v0, 4
    la $a0, elige_vec
    syscall
    li $v0, 5
...
#Eliges el tamaño nuevo del vector
    li $v0, 4
    la $a0, newdim
    syscall
    li $v0, 5
...
cambiar_elem:
# Eliges el vector que quiers cambiar el elemento
    li $v0, 4
    la $a0, elige_vec
    syscall
    li $v0, 5
    ...
    la $a0, newline
    syscall
#Eliges el indice del vector que quieres cambiar
    li $v0, 4
    la $a0, elige_elto
    syscall
...
#(if dim1 != dim2)
    ...
    move $a2, $s2 
    jal prod_esc
#Muevo el flotante que retorna la funcion del producto y la imprimo por pantalla
    mov.s $f12, $f0
...
invertir_vec:
#Imprime en pantalla el vector que quieres invertir
    li $v0, 4
    la $a0, elige_vec
    syscall
    li $v0, 5
    ...
    li $t8, 2
    bgt $t0, $t8, erroropincorrecta
#Opcion de invertir el vector 1
# Si eleccion es igual a 2 salta al if57
    if56: beq $t0, 2, if57
        move $a0, $s0
        move $a1, $s2
        jal mirror
        b Menu
    iffin56:
# Opción de invertir el vector 2
    if57:
        move $a0, $s1
        move $a1, $s3
        jal mirror
...
errordim:
    ...
    syscall
    b Menu
#Error que te indica que el indice que has introducido no esta bien
errorind:
    li $v0, 4
    la $a0, error_ind
    syscall
    b Menu
# Error que indica que la opcion que has escrito no es correcta
erroropincorrecta:
    li $v0, 4
    la $a0, error_op
    syscall
    b Menu
# Error que te indica que las dimensiones no coinciden
errorddim:
    li $v0, 4
    la $a0, error_d_dim
    syscall
